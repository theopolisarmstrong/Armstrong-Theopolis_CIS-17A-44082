 ...
 * Created on March 15th, 2019, 1:00 AM
 * Purpose: Calculate the mode of an array using pointers
 */
...
int *mode(const int *array,int arySize){
    ...
    for (int i = 1; i < arySize; i++){ //Loop through array
        if (ary[i-1] == ary[i]){ //Count matching numbers
            count++;
    ...
    modeAry[0]=modes; //Number of modes in array
    modeAry[1]=maxFreq; //Mode frequency in array
    
    
    //Fill the mode array
    int index = 2; //Index within mode array
    count = 1; //Reset count
    for (int i = 1; i < arySize; i++){ //Loop through array
        if (ary[i-1] == ary[i]){ //Count matching numbers
            count++;
        ...
        if (count == maxFreq){  //Place number into mode array
            modeAry[index] = ary[i]; //Place mode into mode array
            index++;
        }
    }
