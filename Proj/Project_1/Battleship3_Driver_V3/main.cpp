...
Player* initCPU(const char size);
    ...
    while(!gameMode){   //Loop menu until game mode is chosen
        switch(menu(choice)){
            case PVCPU:
                ...
                cout << "Enter map size: ";
                cin >> size;
                cout << "Initializing CPU ship placements...\n";
                cpu = initCPU(size);
...
char menu(char& choice){
    ...
    cout << "0. Player vs. CPU\n";
    cout << "1. Player vs. Player\n";
    cout << "2. Load game from file\n";
    cout << "3. Display last winner\n";
    cout << "9. Exit\n";
...
Player* initCPU(const char size){
    ...
    cpu->ships = new char*[size];
    for (int i = 0; i < size; i++){
        cpu->ships[i] = new char[size];
    }
    //Initialize ships
    for (int i = 0; i < size; i++){
