...
void map(const char ships[][MAP]);   //Output map
void cpuMap(char ships[][MAP], unsigned char type = 1);   //Generate computer player ship positions
...
//bool initFromFile();    //change function identifier
    ...
    while(!gameMode){   //Loop menu until game mode is chosen
        switch(menu(choice)){
            case PVCPU:
                ...
                cpuMap(cShips, CARRIER);
                cpuMap(cShips, DESTROY);
                cpuMap(cShips);
                map(cShips);
...
void cpuMap(char ships[][MAP], unsigned char ship){
